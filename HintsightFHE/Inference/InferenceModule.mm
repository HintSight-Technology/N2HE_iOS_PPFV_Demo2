...
////            cout << t << " "; //print normalised result vector
//            
//            int tt = (int)t;
//            if (t > 0 && t - tt > 0.5) {
...
- (NSArray<NSArray<NSNumber*>*>*)imgFeatureExtractandEnc:(void*)imageBuffer:(NSString*)pkFilePath:(NSString*)mode {
    try {
...
//            cout << t_<< " ";
            double t = t_ / l2norm * scaler0;
//            cout << t << " "; //print normalized result vector
...
//        cout << endl;
        vector<int64_t> input(encDim, 0);
//        cout << "current mode: " << string([mode UTF8String]) << endl;
        if (string([mode UTF8String]) == "register") {
//            cout << "register mode" << endl;
            input[0] = test_vector[0];
            for (int i = 1; i < outputDim; i++) {
                input[encDim-i] = -1 * test_vector[i];
            }
        } else {
            for (int i = 1; i < outputDim; i++) {
...
//        cout << endl;
        ...
        CFTimeInterval startTime1 = CACurrentMediaTime();
        vector<vector<int64_t>> ct = vectorEnc(input, file_path);
        CFTimeInterval elapsedTime1 = CACurrentMediaTime() - startTime1;
        NSLog(@"encryption inference time: %f", elapsedTime1);
